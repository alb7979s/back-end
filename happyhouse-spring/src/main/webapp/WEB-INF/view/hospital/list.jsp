...
<body>
    ...
    <form id="searchform">
	    ...
	    <!--    <button type="button" class="btn dropdown-toggle dropbtn" data-toggle="dropdown">검색</button>-->
	     	<select class="btn dropdown-toggle dropbtn" data-toggle="dropdown" name="key" id="key" onchange="inputPlaceholder();">
	    		...
	    		<option value="medi_name" <c:if test="${result.key =='medi_name'}">selected</c:if>>병원 이름</option>
         ...
         <c:choose>
			<c:when test="${ result.word ne null}">
				<p class="pList">검색하신 [${result.word}]와 관련된 안심병원 목록입니다.</p>
			</c:when>
			<c:when test="${result.city ne null}">
				<p class="pList">관심지역인 [${result.city}, ${result.gugun}]와 관련된 안심병원 목록입니다.</p>
			</c:when>
			<c:otherwise>
				<p class="pList">국민 안심 병원 목록</p>	
			</c:otherwise>
	</c:choose>
    ...
    <c:if test="${result.pageResult.count != 0}"> 
     ...
     function goPage(pageNo){
    	...
    	if(key!='' && word!=''){
    		location.href="${root}/hospital/search?pageNo="+pageNo+"&key="+key+"&word="+word;
    	}
    	else location.href="${root}/hospital/list?pageNo="+pageNo;
     }
     </script>
     ...
     <script type="text/javascript">
		...
		function search(){
			...
			document.getElementById("searchform").action = "${ root }/hospital/search";
